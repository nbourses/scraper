/*esearch.java
 * all the automation of input with the waiting time to work properly
 * Adjust waiting time to work properly
 * select 1st result of search and click on 1st index result
 * new window open with details
 * selenium able to switch on new window and get text (text not supported by eclipse )
 * */
...
public class eserch{
		...
		public static void main(String[] args) throws IOException, RowsExceededException, WriteException, InterruptedException {
			//k for number of results of (Index-II)
			int i=1,j=0,x=1,k=1; 
			boolean br = false,br2;
			//		Create Excel sheet 
			File fExcel = new File ("C:\\Users\\SID\\Desktop\\esearch.xls");
		...
		// Do not change
		  ...
		  outerloop:	   do{   //for Go to next page

		   for(i=2;i<12;i++){  
			   
				 try{ driver.findElement(By.xpath(start+i+end)).isDisplayed();
		 ...
		 //  driver.findElement(By.xpath(start+i+end)).click();
		   ...
		   for(String handle:strHandles){  // Number of Window loop
			   ...
			   {
				   // Excel Title for all the column 
				   if( j<=1){
				...
				//   writableSheet.addCell(data);
				   ...
				   writableSheet.addCell(data);
				//   data = new Label(16,k,driver.findElement(By.xpath("html/body/table[3]/tbody/tr[8]/td[2]/table/tbody/tr[2]/td/font")).getText());
				//   writableSheet.addCell(data);
				   ...
				   writableSheet.addCell(data);
				   data = new Label(22,k,driver.findElement(By.xpath("html/body/table[3]/tbody/tr[14]")).getText());
				   writableSheet.addCell(data);
				   k++;
				   writableBook.write();
		   ...
		   try{br =driver.findElement(By.xpath(nxtPageStart+x+nxtPageEnd)).isDisplayed();
			 System.out.print(br);
		   }catch(Exception e){ }
 		   if (k>=100){br =false;}else{}

		
		 if(x<=13){ 
			try{ driver.findElement(By.xpath(nxtPageStart+x+nxtPageEnd)).click();
